...
import bitsandbytes as bnb  # Import bitsandbytes for 8-bit optimizers
...
def main():
    ...
    model_name = "tiiuae/falcon-7b-instruct"  # Replace with "tiiuae/falcon-7b-instruct" if desired
    ...
    # Note: Skipping QAT due to complexities with large models
    # Proceeding with memory optimizations instead
    ...
    dataloader = DataLoader(dataset, batch_size=1, sampler=sampler)  # Reduced batch size
    ...
    with profile(activities=[ProfilerActivity.CPU, ProfilerActivity.CUDA], record_shapes=True, on_trace_ready=torch.profiler.tensorboard_trace_handler('./log')) as prof:
        ...
        for epoch in range(epochs):
            ...
            if local_rank == 0:
                ...
                # Since QAT is skipped, proceed to save the model normally
